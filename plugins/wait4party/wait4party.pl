...
package wait4party;
        ...
        sub emulateCmdSit {
            ...
            # Find the first valid party member's Actor object
            my $actor;
            foreach my $partyMemberID (@partyUsersID) {
                next if $partyMemberID eq $accountID;
                $actor = $playersList->getByID($partyMemberID);
                last if $actor;
            }
